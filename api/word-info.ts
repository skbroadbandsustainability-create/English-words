  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      tools: [
        {
          name: 'report_word',
          description: '영어 단어 하나의 뜻 정보를 보고한다.',
          input_schema: {
            type: 'object',
            properties: { word: WORD_ITEM_SCHEMA },
            required: ['word'],
          },
        },
      ],
      tool_choice: { type: 'tool', name: 'report_word' },
      messages: [
        {
          role: 'user',
          content: `영어 단어 "${clean}"의 품사, 초등학생 눈높이의 쉬운 영어 설명, 쉬운 한글 뜻, 유의어, 반의어를 정리해줘. 실제 영어 단어가 아니면 word 필드에 입력값을 그대로 담고 definitionEn에 "사전에 없는 단어예요"라고 적어줘.`,
        },
      ],
    })

    const result = extractToolInput<{ word?: AiWord }>(message, 'report_word')
